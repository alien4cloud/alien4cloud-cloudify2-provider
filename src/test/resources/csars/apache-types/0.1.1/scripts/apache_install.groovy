...
config = new ConfigSlurper().parse(new File("${context.serviceDirectory}/scripts/apache-service.properties").toURL())
...
def installLinuxHttpd(context,builder,currVendor,installScript,dbType) {
	builder.sequential {
		echo(message:"apache_install.groovy: Chmodding +x ${context.serviceDirectory}/scripts ...")
		chmod(dir:"${context.serviceDirectory}/scripts", perm:"+x", includes:"*.sh")

		echo(message:"apache_install.groovy: Running ${context.serviceDirectory}/scripts/${installScript} os is ${currVendor}...")
...
def installWindowsHttpd(config,osConfig,downloadFile,builder,dbType) {
	...
	installFolder="${context.serviceDirectory}/scripts/install"
