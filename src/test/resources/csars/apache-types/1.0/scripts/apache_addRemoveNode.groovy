...
def static addOrRemoveNode(config,lifecycleEvent,actionName,seconds2Wait,context,currHttpPort,instanceID) { 
	...
	try {			
		...
		if ( loadbalancerService != null ) { 
			...
			switch(loadBalancerServiceName) {
				case ["apacheLB", ""]:
					if ( "${loadBalancerServiceName}" == "" ) {
						println "load balancer not specified ..."
						println "${myServiceName} by default uses apacheLB"
					}
					loadbalancerService.invoke("${actionName}", currURL as String, instanceID as String)
					break
				case ["haproxy"]:
					loadbalancerService.invoke("${actionName}", "${myServiceName}" as String, "${privateIP}" as String, "${currHttpPort}" as String);
					break
				default:
					throw new Exception("Support for ${loadBalancerServiceName} is not implemented")
			}
		}		
		else {
